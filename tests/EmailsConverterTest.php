...
class EmailsConverterTest extends BearFramework\AddonTests\PHPUnitTestCase
{
    ...
    public function testEmailToRawAndRawToEmail()
    {
        ...
        $this->assertEquals($email->subject, $email2->subject);
        $this->assertEquals($email->date, $email2->date);
        $this->assertEquals($listToArray($email->sender), $listToArray($email2->sender));
        $this->assertEquals($listToArray($email->replyToRecipients), $listToArray($email2->replyToRecipients));
        $this->assertEquals($listToArray($email->bccRecipients), $listToArray($email2->bccRecipients));
        $this->assertEquals($listToArray($email->ccRecipients), $listToArray($email2->ccRecipients));
...
example.com [http://example.com]
 ...
 (html version)';
    ...
    public function testRawToHTML()
    {
        ...
        $expectedHTML = '<!DOCTYPE html>
<html><head><link rel="stylesheet" href="http://ivopetkov.github.io/example-files/example-css.css"><script src="http://ivopetkov.github.io/example-files/example-js.js"></script><style>div{border:1px solid black;}</style></head><body>
        Hi,<br><img src="http://ivopetkov.github.io/example-files/example-image-400-x-300.png"><br><strong>Welcome to our service.</strong><br>
        Best regards,<br>
